...
retry:
	...
	if(i2c_workaround && gReadWriteContext.nNbOfBytesToRead == 1 &&
            gReadWriteContext.nNbOfBytesRead == ERESTART_SNFC_N2)
    {
		LOGE("--------------------->> RX Thread Read Value = %d, Length = %d, i2c_workaround = %d \n"
			 ,  gReadWriteContext.nNbOfBytesRead, gReadWriteContext.nNbOfBytesToRead, i2c_workaround );
		
        phOsalNfc_RaiseException(phOsalNfc_e_UnrecovFirmwareErr,0);
	}
    else
    {
        i2c_error_count = 0;
...
#ifdef CONFIG_SNFC	
				if(i2c_workaround &&  gReadWriteContext.nNbOfBytesWritten == ERESTART_SNFC_N2)
				{
					LOGE("--------------------->> TX Thread Write Value = %d, Length = %d, i2c_workaround = %d \n"
						 ,  gReadWriteContext.nNbOfBytesWritten, gReadWriteContext.nNbOfBytesToWrite, i2c_workaround );
					
					phOsalNfc_RaiseException(phOsalNfc_e_UnrecovFirmwareErr,11);
				}
#endif				
            }
            else
