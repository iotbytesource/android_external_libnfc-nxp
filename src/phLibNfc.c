                                ...
                                )
{
    ...
    else
    {
            ...
            (NFCSTATUS_BOARD_COMMUNICATION_ERROR == status))
        {
            ...
            phOsalNfc_RaiseException(phOsalNfc_e_UnrecovFirmwareErr,4);
        }
    }
    return;
}
                    ...
                    void                         *pContext)
{
	...
	{
#ifdef SNFC_FW_DOWNLOAD
	        phLibNfc_StackCapabilities->psDevCapabilities.firmware_update_info = memcmp(phLibNfc_StackCapabilities->psDevCapabilities.full_version, snfc_fw_version,
	                   SNFC_FW_VERSION_LEN);
	      
		LOGD("Internal FW : %02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x",
		phLibNfc_StackCapabilities->psDevCapabilities.full_version[0], phLibNfc_StackCapabilities->psDevCapabilities.full_version[1], 
		phLibNfc_StackCapabilities->psDevCapabilities.full_version[2], phLibNfc_StackCapabilities->psDevCapabilities.full_version[3],
		phLibNfc_StackCapabilities->psDevCapabilities.full_version[4], phLibNfc_StackCapabilities->psDevCapabilities.full_version[5], 
		phLibNfc_StackCapabilities->psDevCapabilities.full_version[6], phLibNfc_StackCapabilities->psDevCapabilities.full_version[7]);

		LOGD("External FW : %02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x",
		snfc_fw_version[0], snfc_fw_version[1], 
		snfc_fw_version[2], snfc_fw_version[3],
		snfc_fw_version[4], snfc_fw_version[5], 
		snfc_fw_version[6], snfc_fw_version[7]);
#else
		phLibNfc_StackCapabilities->psDevCapabilities.firmware_update_info = 0;
#endif		
	}
