...
#ifdef CONFIG_SNFC
				if((psHciContext->hci_state.cur_state == hciState_Reactivate) && (error_status == 0x889))
				{
					LOGE ("----------------------------------------------------------------------------------------- \n");
					LOGE ("HCI Err : Returned NFC F/W Timeout Response \n");
					LOGE ("HCI Err : Reset NFC System  \n");
					phOsalNfc_RaiseException(phOsalNfc_e_UnrecovFirmwareErr,3);
				}
				else
				{
