...
#ifdef CONFIG_SNFC
#define FW_PATH "/system/lib/libsnfc_fw.so"

const unsigned char *snfc_fw_version;
int snfc_fw_line;
const unsigned char (*snfc_fw_data)[SNFC_FW_HEX_ROW];

int snfc_dlopen_firmware()
{
    void *p;
    void *handle;

    handle = dlopen(FW_PATH, RTLD_NOW | RTLD_LOCAL);
    if (handle == NULL) {
        LOGE("Could not open %s", FW_PATH);
        return -1;
    }

    p = dlsym(handle, "fw_line");
    if (p == NULL) {
        LOGE("Could not link fw_line");
        return -1;
    }
    snfc_fw_line = *(int *)p;

    p = dlsym(handle, "fw_version");
    if (p == NULL) {
        LOGE("Could not link fw_version");
        return -1;
    }

    snfc_fw_version = (unsigned char *)p;

    p = dlsym(handle, "fw_data");
    if (p == NULL) {
        LOGE("Could not link fw_data");
        return -1;
    }
    
    snfc_fw_data = (unsigned char (*)[SNFC_FW_HEX_ROW])p;
    ...
    else
    {
        ...
        switch(type)
        {
            ...
            case NFC_NOTIFY_DEVICE_ERROR:
            {
                ...
                {
                    Hal4Ctxt->Hal4NextState = eHal4StateInvalid;
                    phOsalNfc_RaiseException(phOsalNfc_e_UnrecovFirmwareErr,2);
                }
                break;
            }
